...
export default async function handler(req, res) {
    if (req.method === 'GET') {
        try {
            ...
            const users = await User.find({ name: { $regex: `^${slug}`, $options: 'i' } }); // Case-insensitive search

            // Query the database for details associated with the found users
            const details = await Details.find({ user: { $in: users.map(user => user._id) } });


            // Return the matching users and their details
            return res.status(200).json({ users, details });
        } catch (error) {
            console.error(error);
